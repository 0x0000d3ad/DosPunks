...
module.exports = async function( callback ) {
	...
	console.log( "Get other (minter) balance token 1: should be 0" ); 
	let res01 = await token.balanceOf( owner, token1, { "from" : other } );
	console.log( res01 );
	console.log( "Get other (minter) balance token 2: should be 0" ); 
	let res02 = await token.balanceOf( owner, token1, { "from" : other } );
	console.log( res02 );
	console.log( "Get other (minter) balance token 3: should be 0" ); 
	let res03 = await token.balanceOf( owner, token1, { "from" : other } );
	console.log( res03 );

	console.log( "Get burn balance token 1: should be 1" ); 
	let res11 = await token.balanceOf( burn, token1, { "from" : other } );
	console.log( res11 );
	console.log( "Get burn balance token 2: should be 1" ); 
	let res12 = await token.balanceOf( burn, token1, { "from" : other } );
	console.log( res12 );
	console.log( "Get burn balance token 3: should be 1" ); 
