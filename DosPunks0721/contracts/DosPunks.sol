...
M""""""'YMM MMP"""""YMM MP""""""`MM MM"""""""`YM                   dP                
M  mmmm. `M M' .mmm. `M M  mmmmm..M MM  mmmmm  M                   88                
M  MMMMM  M M  MMMMM  M M.      `YM M'        .M dP    dP 88d888b. 88  .dP  .d8888b. 
M  MMMMM  M M  MMMMM  M MMMMMMM.  M MM  MMMMMMMM 88    88 88'  `88 88888"   Y8ooooo. 
M  MMMM' .M M. `MMM' .M M. .MMM'  M MM  MMMMMMMM 88.  .88 88    88 88  `8b.       88 
M       .MM MMb     dMM Mb.     .dM MM  MMMMMMMM `88888P' dP    dP dP   `YP `88888P' 
...
contract DosPunks is ERC721Enumerable, Ownable {
	...
	// test baseURI
	string public baseURI = "https://xdwz76itvztyklm2iorkofvebbsjsknixkbb3eums5oiqkpyje5q.arweave.net/uO2f-ROuZ4UtmkOipxakCGSZKai6gh2SjJdciCn4STs/";

	string public baseExtension = ".json";
	uint256 public maxSupply = 500;
	bool public paused = false;
	string not_owner_error = "You are not the owner of this token on OpenSea";
  
	address public withdrawal_address = 0x405874F1Ce5778d5FFDf66f7aba14DA446564B6a;
	address public dospunks_address = 0xD1619BCAbc52DADd3068d7cDBD555a7F2d0C575b;
	...
	external {
		...
		require( IERC1155( dospunks_address ).balanceOf( msg.sender, _tokenIdERC1155 ) == 1, not_owner_error );
	...
	external {
		...
		for ( uint256 i = 0; i < length; i++ ) {
			// Require that the sender of the transaction is the owner of the token on OpenSea
			require( IERC1155( dospunks_address ).balanceOf( msg.sender, _tokenIdsERC1155[ i ] ) == 1, not_owner_error );

			// get token id
			uint256 tokenId = getTokenId( _tokenIdsERC1155[ i ] );
		...
		// batch burn
		IERC1155( dospunks_address ).safeBatchTransferFrom( msg.sender, burn_address, _tokenIdsERC1155, amounts, "");

		emit migrateTokensEvent( _tokenIdsERC1155 );
	}
